...
from src.models import AnalysisReport, FileChange, PRMetadata
...
def run(pr_url: str, github_token: str = "") -> AnalysisReport:
    ...
    1. 解析 URL → 拉取数据
    2. Summary（全量文件）
    3. Risk + Suggestion（并行，仅代码文件）
    4. Context（全量文件 + GitHub 数据）
    5. 聚合 + 去重 + 置信度
    """
...
def run_and_save(pr_url: str, github_token: str = "", output_dir: str = ".") -> AnalysisReport:
